...
- **Change-view foundation.** Every shared link is stamped with `atlasDataset`, a 12-character identity of the accepted dataset (a digest of the node and edge collections). Opening a link made with a different data version keeps that identity through navigation and says only that the version differs and results may differ. Nothing claims which version is newer. No comparison control exists; saved-view comparison is #262.
