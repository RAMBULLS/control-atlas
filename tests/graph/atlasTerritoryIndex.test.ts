...
const input: TerritoryBuildInput = {
  ...
  identities: read("data/generated/publication-identity-index.json").identities, datasetId: "0123456789ab", taxonomy: read("data/generated/taxonomy-registry.json"), sources: read("data/source-registry.json").sources, registryPublications: read("data/source-registry.json").publications, nodes, edges,
};
...
test("the index is small, deterministic and validates against its manifest", () => {
  ...
  const manifest = validateTerritoryManifest({ schemaVersion: 3, datasetId: "0123456789ab", generatedAt: index.generatedAt, sha256: "a".repeat(64), bytes: Buffer.byteLength(text),
...
test("validation rejects a wrong release, a missing publication and a bad route", () => {
  const text = JSON.stringify(index);
  const base = { schemaVersion: 3, datasetId: "0123456789ab", generatedAt: index.generatedAt, sha256: "b".repeat(64), bytes: text.length, publicationCount: index.publications.length, routeCount: index.routes.length, admittedEdgeCount };
